...
function getData(el) {
  return el?.__reactInternalInstance?.memoizedProps?.[KEY_PROPS_DATA] ?? 
         getComponentData(el) ?? 
...
const ReactInspectorOverlay = () => {
  ...
  const logoColors = useMemo(() => {
    return enabled ? ['#42D392', '#213547', '#42b883'] : ['#E2C6C6', '#E2C6C6', '#E2C6C6'];
  }, [enabled]);
  ...
  const updateLinkParams = useCallback((e) => {
    const { targetNode, params } = getTargetNode(e);
    if (targetNode) {
  ...
  const openInEditor = useCallback((baseUrl, file, line, column) => {
  ...
  }, [enabled, position, linkParams, enable, disable, toggleEnabled, onEnabled, onDisabled, openInEditor, onUpdated]);
  ...
  return (
    <div {...{ [KEY_IGNORE]: 'true' }}>
      {containerVisible && (
        <div
          ref={containerRef}
          className={`react-inspector-container ${!enabled ? 'react-inspector-container--disabled' : ''}`}
          style={{ 
            position: 'fixed',
            cursor: 'pointer',
            textAlign: 'center',
            ...
            ...Object.fromEntries(containerPosition.split(';').filter(Boolean).map(s => s.split(': '))),
          }}
        >
          {/* Logo */}
          <svg
          ...
          >
            ...
            <defs>
              ...
              >
                <stop offset="0.515625" stopColor={logoColors[0]} />
                <stop offset="0.552083" stopColor={logoColors[2]} />
              </linearGradient>
            </defs>
          </svg>
          ...
          >
            ...
            <div style={{ fontSize: '11px', opacity: 0.7 }}>Click on a element › Open IDE › Link to File</div>
          </a>
        </div>
      )}
